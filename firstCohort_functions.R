...
)
{
    if (GENE){
        p <- ggplot(
            ...
            aes(
                x=DF[, X], y=DF[, Y],
                shape=ANNO[, SHAPE], colour=COLOUR
            )
        )
    ...
    {
        colorPal <- c(brewer.pal(n = 5, name = "Dark2"), brewer.pal(n = 4, name = "Paired"))
...
plot.violin <- function(DF, ANNO, Y, X='Source', FILL=NULL, FILL.LAB="Add a label", JITTER=FALSE, GENE=TRUE)
{
    DF <- as.data.frame(DF)
    if (is.null(FILL))
    {
    ...
    if (GENE){p <- p + theme(axis.title.y = element_text(size=14, colour='black', face="italic"))}
...
plot.pie <- function(ANNO, VAR='Ig.class', LEGEND.TITLE="Ig class")
{
    if (VAR == "Ig.class")
    {
        colorPal <- c("IGHA1" = "gold1", "IGHA2" = "gold3", "IGHG1" = "brown1", "IGHG2" = "brown2", "IGHG3" = "brown3", "IGHG4" = "brown4", "IGHE" = "dodgerblue3", "IGHM" = "forestgreen", "IGHD" = "chartreuse3")
        labOrder <- c("IGHM", "IGHD", "IGHA1", "IGHA2", "IGHG1", "IGHG2", "IGHG3", "IGHG4", "IGHE")
        ANNO[, VAR] <- factor(ANNO[, VAR], levels=labOrder)
    } else if (class(ANNO[, VAR]) != "factor") {stop("VAR needs to be a factor.")}
